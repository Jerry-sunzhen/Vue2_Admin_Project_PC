...
mounted中请求一级分类
监听一级分类的onChange事件,当值改变的时候进行二级分类请求并渲染
...
* 如果没有选择指定的三级分类,添加属性的按钮应该被禁用
* 当需要添加的属性名没有值的时候,添加属性值的按钮要被禁用; 当新增的属性名与属性值都有值的时候,保存属性名的按钮才可以被启用
